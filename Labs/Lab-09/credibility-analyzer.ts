...
7. Call evaluate_credibility exactly once with a complete structured assessment grounded only in evidence you actually found.
8. After evaluate_credibility succeeds, call write_file to write the final Markdown report.
9. Then respond briefly with the file path and the overall verdict.
...
- Use evaluate_credibility before write_file.
...
function createTools(tvly: ReturnType<typeof tavily>) {
    ...
    async execute({ url }) {
      ...
      try {
        ...
        const response = await fetch(`https://r.jina.ai/${url}`, {
          headers: {
            Accept: 'text/plain',
            'User-Agent': 'lab-09-credibility-analyzer/1.0',
          },
  ...
  const evaluateCredibilityTool = tool({
    name: 'evaluate_credibility',
    description:
      'Record your full structured credibility assessment after investigating the source, author, publication, corroboration, bias, and currency. Every field must be based on evidence you actually found.',
  ...
  const writeFileTool = tool({
    name: 'write_file',
    description:
      'Write the final Markdown credibility report to disk inside the local reports directory. Use this only after evaluate_credibility has been called.',
    parameters: WriteFileArgsSchema,
    async execute({ filename, content }) {
      if (!lastEvaluation) {
        throw new Error('Call evaluate_credibility before write_file so the final report is grounded in a completed structured assessment.');
      }
  ...
  return [readUrlTool, webSearchTool, evaluateCredibilityTool, writeFileTool] as const;
}
...
async function main() {
  ...
  const openRouterClient = new OpenAI({
    ...
    defaultHeaders: {
      ...
      'X-Title': process.env.OPENROUTER_APP_TITLE || 'lab-09-credibility-analyzer',
    },
  });
  ...
  const runner = new Runner({
    ...
    workflowName: 'Lab 09 Research Source Credibility Analyzer',
  });
  ...
  const evaluation = lastEvaluation as CredibilityEvaluation | null;
  if (!evaluation) {
    throw new Error('The run finished without calling evaluate_credibility.');
  }
  ...
  const summaryLines = [
    `Report: ${reportPath}`,
    `Trace: ${tracePath}`,
    `Verdict: ${evaluation.overall_credibility}`,
  ];
