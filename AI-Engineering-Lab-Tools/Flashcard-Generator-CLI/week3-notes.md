...
> [!TIP]
> See the sample code, [cost.js](../../AI-Engineering-Lab-Tools/Flashcard-Generator-CLI/cost.js) and [cost.py](../../AI-Engineering-Lab-Tools/Flashcard-Generator-CLI/cost.py) in the Lab 2 materials, for calculating token usage and cost for OpenRouter models and chat completions.
...
Because LLMs have to respond with something, and since there are many ways that they _could_ respond, it's important to determine what would be useful for the given interaction:
...
When a model struggles to understand your instructions, or consistently fails to output the specific format you need, **Few-Shot Prompting** is usually the next solution. "Few-shot" simply means providing a _few_ examples (i.e., shots) of the task being performed correctly within the prompt context.
...
LLMs can be sensitive to specific phrasing. A vague question will often yield a vague answer. RaR instructs the model to first re-interpret the user's intent, expand on it, and then answer the _improved_ version of the question.
